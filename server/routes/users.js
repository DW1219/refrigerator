...
router.post("/addToCart", auth, (req, res) => {
    ...
    User.findOne({ _id: req.user._id },     //findOne은 몽고DB에서 제공하는 메소드 // 원형은 db.collection.findOne(query, projection)  //이 id로 데이터들을 찾기위함
        (err, userInfo) => {                //2번째 인자인 projection 필드(userInfo)에 값이 포함된 채로 리턴이 되는 것.

            let duplicate = false;          //임시 flag (같은게 있는지 여부 확인을 위해)
            userInfo.cart.forEach((item) => {       //cart에는 여러개가 있을수 있으므로 forEach로 여러번 돌려봐야함
                if (item.id === req.body.productId) {      //같은게 하나라도 있으면
                    duplicate = true;                      //있다고 표시
                }
            })
            if (duplicate) {     //상품이 있다면 cart 쪽에 quantity 만 1개 추가
                User.findOneAndUpdate(                //마찬가지 몽고DB 제공 메소드. 원형: findOneAndUpate(filter,update,options)
                    ...
                    (err, userInfo) => {              // 다시 받아온 업데이트 정보를 가지고 front-end(client)쪽에 리턴해주어야함.
                        if (err) return res.status(400).json({ success: false, err })
                        return res.status(200).send(userInfo.cart)      //front-end 쪽에는 cart 정보만 주면 됨
                    }
                )
            } else {             //상품이 없다면 id와 장바구니수량(quantity)과 날짜(date)를 넣어줘야한다.
                console.log("req.user._id : " + req.user._id)
                User.findOneAndUpdate(
                    { _id: req.user._id },
                    {
                        $push: {
                            cart: {                //cart 안에 정보들을 넣을때 이런식으로 넣는다.
    ...
    User.findOneAndUpdate(
        ...
        (err, userInfo) => {
            ...
            Product.find({ _id: { $in: array } })              //다시 추출한것을 새로 넣는 것임....
                .populate('writer')
                .exec((err, productInfo) => {                          //productInfo를 받았다면 다시 client쪽으로 보냄
...
router.post("/successBuy", auth, (req, res) => {
    ...
    req.body.cartDetail.forEach((item => {             //이미 action 넘길때 인자로 cartDetail: props.user.cartDetail 라고 redux의 구조를 받았음.
        tempHistory.push({
    ...
    transactionData.user = {     // 첫째로 user[] 구성하기
    ...
    User.findOneAndUpdate(
        ...
        (err, userInfo) => {
            ...
            payment.save((err, doc) => {
                ...
                async.eachSeries(tempProducts, (item, callback) => {
                    Product.update(
                        { _id: item.id },         //id로 찾고
                        {
                            $inc: {
                    ...
                    if (err) return res.status(400).json({ success: false, err })
                    return res.status(200).json({ success: true, cart: userInfo.cart, cartDetail: [] })  //cartDetial은 비워주는것. cart는 임의이름. reducer쪽에 payload.cart 로만 넘겨주면됨
                }
                )
