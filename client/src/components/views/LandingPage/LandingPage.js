...
function LandingPage() {
    ...
    useEffect(() => {        //useEffect 함수는 리액트 컴포넌트가 렌더링 될때마다 특정작업을 실행할 수 있도록 하는 Hook
    ...
    const getProducts = (body) => {
        axios.post('/api/product/products', body)              //여긴 보내는쪽Client. 서버쪽 routes 밑에 product 쪽에 받는부분 구현해야함 (body 같이 보내줌)
            .then(response => {
    ...
    const renderCards = Products.map((product, index) => {
        ...
        return <Col lg={6} mg={8} xs={24} key={index}>
    ...
    const handlePrice = (value) => {
        ...
        for (let key in data) {                             //key는 data의 맨앞부분인 id값들이 될것... 0,1,2,3,4....

            if (data[key]._id === parseInt(value, 10)) {         // 그냥 data[key]._id === value 로 해도되는데 만약을 위해 10진수로 변환
    ...
    const handleFilters = (filters, category) => {              // regions 하고 price 필터구분을 모두 받을 용도로 공용할 것이므로 category 라고 명칭
        ...
        if (category === "price") {                             // Price일 경우에는 index가 아닌 가격조건(0,9999 등)으로 구분해서 이미지를 뿌려줘야하므로,
    ...
    const updateSearchTerm = (newSearchTerm) => {    // props.refreshFunction(event.currentTarget.value) 를 받은 데이터
    ...
    return (
        <div style={{ width: '75%', margin: '3rem auto' }}>
            <div style={{ textAlign: 'center' }}>
                <h2>상품 List <Icon type="rocket" /></h2>
            </div>
            {/* Filter */}
            <Row gutter={[16, 16]}>
                <Col lg={12} xs={24}>
                    {/* CheckBox */}
                    <Checkbox list={regions} handleCommunication={filters => handleFilters(filters, "regions")} />
                </Col>
                <Col lg={12} xs={24}>
                    {/* RadioBox */}
                    <Radiobox list={price} handleCommunication={filters => handleFilters(filters, "price")} />
                </Col>
            </Row>
