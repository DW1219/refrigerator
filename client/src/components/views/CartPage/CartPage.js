...
function CartPage(props) {                         //여기 props는 현재 redux 최상위 root 경로임
    ...
    useEffect(() => {
        ...
        if (props.user.userData && props.user.userData.cart) {    //userData가 있고 cart까지 있다면
            if (props.user.userData.cart.length > 0) {
                ...
                dispatch(getCartItems(cartItems, props.user.userData.cart))      //상품id집합 과 cart 정보를 인자로 넘긴다.  
                    .then(response => { calculateTotalPrice(response.payload) })   //상품합계 구하는 함수 실행(인자로 전체 데이터 넘김)
            }
        }
    ...
    let calculateTotalPrice = (cartDetail) => {
        ...
        cartDetail.map(item => {                                //왠만한건 다 map으로 가져옴
    ...
    let removeFromCart = (productId) => {
        dispatch(removeCartItem(productId))                  //Action (removeCartItem) 정의필요
            .then(response => {
                ...
                if (response.payload.productInfo.length <= 0) {  //하나도 상품이 없을경우
                    setShowTotal(false)
                }
            })
    }
    ...
    const transactionSuccess = (payment) => {
        ...
        dispatch(onSuccessBuy({                             //2개의 인자를 서버로 넘김 1.paypal 리턴받은 데이터,  2. 리덕스 store의 cartDetail 정보
            paymentData: payment,
